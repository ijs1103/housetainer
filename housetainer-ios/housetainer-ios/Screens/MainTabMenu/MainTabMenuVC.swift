...
final class MainTabMenuVC: UIViewController {
    ...
    private lazy var buttonDictionary: [Category: (button: CategoryButton, fetchFunction: (() -> Void))] = [
        .currentEvents: (currentEventsButton, { Task { await self.viewModel.filterEvents(isPast: false) } }),
        .pastEvents: (pastEventsButton, { Task { await self.viewModel.filterEvents(isPast: true) } }),
    ...
    override func viewDidLoad() {
        ...
        Task {
            if isSocialCalendarTapped {
                await viewModel.filterEvents(isPast: false)
            } else {
...
extension MainTabMenuVC: UICollectionViewDelegate {
    ...
    //TODO: UICollectionView에 데이터 4개씩 fetchMore하는 무한스크롤 적용, 기존의 fetch로직도 변경
}
